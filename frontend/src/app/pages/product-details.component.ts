            ...
            <div class="container mx-auto px-4 py-8">
              <div class="flex flex-col lg:flex-row gap-10">
                ...
                <div class="lg:w-1/2 space-y-6">
                  ...
                  <!-- Avis Clients -->
                  <div class="mt-12 space-y-6 pt-8 border-t border-gray-100">
                    <h3 class="text-lg font-bold text-gray-900 uppercase tracking-wider">{{ lang.isRTL() ? 'آراء العملاء' : 'Avis Clients' }}</h3>
                    
                    <div *ngIf="!product.reviews || product.reviews.length === 0" class="text-center py-8 bg-gray-50 rounded-2xl border border-dashed border-gray-200">
                      <p class="text-gray-500 font-medium italic">{{ lang.isRTL() ? 'لا توجد آراء حتى الآن' : 'Aucun avis pour le moment.' }}</p>
                    </div>

                    <div *ngIf="product.reviews && product.reviews.length > 0" class="space-y-4">
                      <div *ngFor="let review of product.reviews" class="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm">
                        <div class="flex items-center justify-between mb-3">
                          <span class="font-bold text-gray-900">{{ review.name }}</span>
                          <div class="flex text-yellow-400">
                            <svg *ngFor="let s of [1,2,3,4,5]" class="h-4 w-4" viewBox="0 0 20 20" fill="currentColor"
                                 [class.text-yellow-400]="s <= review.rating"
                                 [class.text-gray-200]="s > review.rating">
                              <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/>
                            </svg>
                          </div>
                        </div>
                        <p class="text-gray-600 text-sm whitespace-pre-wrap">{{ review.comment }}</p>
                        <p class="text-[10px] text-gray-400 mt-2" *ngIf="review.createdAt">{{ review.createdAt | date:'shortDate' }}</p>
                      </div>
                    </div>
                  </div>
                </div>
